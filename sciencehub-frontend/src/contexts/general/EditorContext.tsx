...
import { ProjectSubmissionSmall, WorkSubmission } from "@/src/types/versionControlTypes";
import { Work, WorkIdentifier } from "@/src/types/workTypes";
...
export type EditorContextType = {
    ...
    setOpenedWorkIdentifiers: (
        workIdentifiers: Record<number, Record<number, WorkIdentifier>> | undefined
    ) => void;
...
/**
 * Context for the UnifiedEditor. Not in use until UnifiedEditor is fully implemented.
 */
...
export const EditorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const [openedProject, setOpenedProject] = useState<ProjectSmall | undefined>();
    const [projectDirectory, setProjectDirectory] = useState<ProjectDirectory | undefined>();

    const [activeWindows, setActiveWindows] = useState<number[]>([]);
    const [openedWorks, setOpenedWorks] = useState<Record<number, Record<number, Work>>>();
    const [openedWorkIdentifiers, setOpenedWorkIdentifiers] =
        useState<Record<number, Record<number, WorkIdentifier>>>();
