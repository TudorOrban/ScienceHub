...
export const fetchGeneralData = async ({
    ...
    relationships, // no role currently, ignore
    options = {
        // search options
        inputQuery: "",
        filters: {}, // filters
...
//     // Only fetch the total count if withCounts is true
//     if (withCounts) {
//         const countResponse = await supabase
//             .from(tableName)
//             .select("id", { head: true, count: "exact" });
//         const totalCount = parseInt(countResponse.count?.toString() || "0");
//         // Attach totalCount as a property to the finalData array
//         (finalData as any).totalCount = totalCount;

//   // Log to check if totalCount is set
//   console.log("Final data:", finalData);
//   console.log("Total count:", (finalData as any).totalCount);
//     }
