...
export const flattenWorks = (worksData: HookResult<UserAllWorksSmall>): WorkSmall[] => {
    const experiments =
        (worksData.data[0]?.experiments || []).map((experiment) => {
    ...
    const datasets =
        (worksData.data[0]?.datasets || []).map((dataset) => {
            const work: WorkSmall = {
                id: dataset.id,
                title: dataset.title,
                workType: "Dataset",
    ...
    const dataAnalyses =
        (worksData.data[0]?.dataAnalyses || []).map((dataAnalysis) => {
            const work: WorkSmall = {
                id: dataAnalysis.id,
    ...
    const aiModels =
        (worksData.data[0]?.aiModels || []).map((aiModel) => {
            const work: WorkSmall = {
                id: aiModel.id,
                title: aiModel.title,
                workType: "AI Model",
    ...
    const codeBlocks =
        (worksData.data[0]?.codeBlocks || []).map((codeBlock) => {
            const work: WorkSmall = {
                id: codeBlock.id,
