...
export default function ResearchPage({
    ...
    const multiWorks: MultiWorks = {
        experiments: userWorks.data?.[0]?.experiments?.map((experiment) => (
            transformWorkToWorkInfo(experiment, undefined))) || [],
        datasets: userWorks.data?.[0]?.datasets?.map((dataset) => (
            transformWorkToWorkInfo(dataset, undefined))) || [],
        dataAnalyses: userWorks.data?.[0]?.dataAnalyses?.map((dataAnalysis) => (
            transformWorkToWorkInfo(dataAnalysis, undefined))) || [],
        aiModels: userWorks.data?.[0]?.aiModels?.map((aiModel) => (
            transformWorkToWorkInfo(aiModel, undefined))) || [],
        codeBlocks: userWorks.data?.[0]?.codeBlocks?.map((codeBlock) => (
            transformWorkToWorkInfo(codeBlock, undefined))) || [],
        papers: userWorks.data?.[0]?.papers?.map((paper) => (
