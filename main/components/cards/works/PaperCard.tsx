...
const PaperCard: React.FC<PaperCardProps> = (props) => {
    ...
    return (
        <div className="border border-gray-300 rounded-lg  shadow-md w-full">
            ...
            <div className="flex justify-between items-start p-4 bg-gray-50 border-b border-gray-300">
                <div>
                    ...
                    <div className="flex items-center text-gray-600 text-lg ml-2 flex-wrap">
                        ...
                        {(paper.users || []).map((user, index) => (
                            <Link
                                key={index}
                                href={`/${user.username}/profile`}
                            >
                                <span className="ml-1 text-blue-600 block">
                                    {index !== (paper.users || []).length - 1
                                        ? `${user.fullName}, `
                                        : user.fullName}
                                </span>
                            </Link>
                    ...
                    <div className="flex justify-end first-letter:m-2 pb-4 mr-2 mt-2">
                        <FeatureBox
                            feature={{
                                ...
                                value: paper.researchScore?.toString() || "0",
                            }}
                        />
                        <FeatureBox
                            feature={{
            ...
            {/* Description */}
            <WorkField title="Description" content={paper.description || ""} />
            ...
            {paper.supplementaryMaterial && (
                <div className="border-b border-gray-300 px-4 py-3">
                    <h3 className="font-medium text-lg mb-2">
                        Supplementary Material:
                    </h3>
                    <Link href="#">{paper.supplementaryMaterial}</Link>
                </div>
            )}
            {/* License */}
            {paper.license && (
